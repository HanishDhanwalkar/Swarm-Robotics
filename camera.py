...
def detect(image):
    ...
    box = [0, 0, 0, 0]
    for i in range(len(markerCorners)):
        image = cv.polylines(image, markerCorners[i].astype(
            'int32'), True, (255, 0, 0), 2)
        image = cv.putText(image, str(markerIds[i][0]), markerCorners[i].mean(axis=1)[0].astype(
            'int32'), cv.FONT_HERSHEY_SIMPLEX, color=(0, 0, 255), thickness=1, fontScale=0.3)
        try:
            box[order.index(markerIds[i][0])] = markerCorners[i].mean(
                axis=1)[0].astype('int32')
        except:
            continue
    try:
...
class MobileCamera:
    ...
    def getCorners(self, image):
        ...
        for i in range(len(markerCorners)):
            try:
                detectedCorners[order.index(markerIds[i][0])] = markerCorners[i].mean(axis=1)[
                    0].astype('int32')
            except:
                continue
