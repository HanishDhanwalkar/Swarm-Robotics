...
class Map:
    ...
    def isCollision(self,p,point:Node):
        ...
        # print(t)
        if self.mapT[p[0]+point.x-3, p[1]+point.y-3]==np.inf  or abs(self.mapT[p[0]+point.x-3, p[1]+point.y-3]-t)<3:
    ...
    def neighbourOfObstacle(self, point):
        ...
        if len(obstacles) == 0:
            return 0
        closest = sorted(list(obstacles), key=lambda x: self.isCollision(x,point))[0]
        return 100/self.Dist(closest, np.array([3, 3]))

    def updatedGScore(self, init:Node , new:Node ):
        t=init.t+init.Dist(new)/3
        if self.map[new.x, new.y] == 100 and (self.mapT[new.x, new.y]==np.inf  or abs(self.mapT[new.x, new.y]-t)<3):
            return np.inf
